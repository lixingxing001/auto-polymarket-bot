...
- market-aware 回测只允许模型用训练段学习，再对后续窗口做预测
- 可以额外设置最低置信度门槛，避免把低质量样本塞回交易集合
...
- 每隔固定秒数记录当前窗口的 `best bid / best ask`
- 同时保存窗口 slug、condition id、token id 与距离结算的秒数
- 未来回测优先使用自有快照还原当时真实可见盘口

## 快照驱动回测

- 在决策时点之后的短窗口内寻找第一条可见盘口快照
